...
- **Figure layout (specific to the 2x2/quarters default, not a generic N x M layout)**: a big display of the whole raw micrograph in the center; each quadrant's rose + tortuosity-hist + curvature-hist cluster in the figure corner matching that quadrant's actual position in the source image (top-left quadrant's cluster in the figure's top-left corner, etc.); the whole image's *own* rose/tortuosity/curvature cluster (its analysis result, distinct from the raw-image display above it) in a row below everything. An earlier flat-list-of-rows layout (one row per section) was tried first and replaced -- 16 sections (the original 4x4 default) made for a tall, scroll-heavy figure; quarters plus this spatial layout reads much faster.
