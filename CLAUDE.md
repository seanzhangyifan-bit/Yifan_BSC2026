...
- **`scripts/overview_figure.py <image_path> [--n-rows 4] [--n-cols 4] [--out-dir outputs]`**: times one pass of the curvature/anisotropy pipeline slice (binarize -> skeletonize -> graph -> curvature -> anisotropy -- deliberately *not* junctions/kinks/corner-cross-check, which this doesn't need) over the whole image, then again over a coarse `n_rows x n_cols` grid of non-overlapping sections (`src/crackgraph/tiling.py`'s `grid_tiles`), and compiles every section's rose diagram + curvature/tortuosity histogram, plus the whole-image result, into one figure (`src/crackgraph/summary_plots.py`'s `render_overview_figure`): one row per section (rose | tortuosity-hist | curvature-hist), whole-image result as the final row.
- **Decisions already made, so a future session doesn't silently redo them differently**: the grid is a coarse, independent tiling (default 4x4 = 16 sections) decoupled from `region.py`'s `corner_frac` -- tiling at corner-crop size would give ~50-100+ tiles, too many to read in one figure. The panel layout is a flat list of rows, not a spatial contact-sheet matching each section's position in the image (considered, not built). `grid_tiles` does **not** apply `edge_margin_frac` -- every pixel is covered, edge to edge.
