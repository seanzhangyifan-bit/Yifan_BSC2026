...
def test_overview_figure_renders_one_row_per_section_plus_whole_image(tmp_path):
    ...
    sections = [("section r0c0", aniso_a, curv_a), ("section r0c1", aniso_b, curv_b)]
    whole_image = ("whole image", aniso_a, curv_a)
    out_path = tmp_path / "overview.png"

    render_overview_figure(sections, whole_image, out_path)
...
def test_overview_figure_handles_empty_sections_list(tmp_path):
    gray = generate_oriented_segment_field(bearings_deg=[0.0], n_segments=40, rng_seed=1)
    curv, aniso = _run_curvature_and_anisotropy(gray)
    whole_image = ("whole image", aniso, curv)
    out_path = tmp_path / "overview_empty.png"

    render_overview_figure([], whole_image, out_path)
